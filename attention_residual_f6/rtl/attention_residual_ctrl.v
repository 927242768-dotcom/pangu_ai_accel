...
// layer0 Attention 最终残差的 UART、DDR3 和计算调度控制器。
//
// UART 协议（115200 8N1）：
//   I -> "PANGU50K ATTN RESIDUAL V1\r\n"
//   S -> 'S' + flags + "\r\n"
//   L + 8960 B 固定载荷 -> "K\r\n"
//   G -> 计算并回复 'R' + 896 个 little-endian signed int16 Q6.10
//
// 上传载荷：
//   residual_hidden_q10[896] : 1792 B / 56 拍
//   oproj_q28[896]           : 7168 B / 224 拍
module attention_residual_ctrl #(
...
localparam integer HIDDEN_BEATS     = 56;
localparam integer OPROJ_BEATS      = 224;
localparam integer RESULT_BEATS     = 56;
localparam integer LOAD_TOTAL_BEATS = HIDDEN_BEATS + OPROJ_BEATS;

// DDR3 Controller 地址单位为 32 bit；每个 256 bit beat 地址递增 8。
localparam [CTRL_ADDR_WIDTH-1:0] ADDR_HIDDEN = 28'h0000000;
localparam [CTRL_ADDR_WIDTH-1:0] ADDR_OPROJ  = 28'h0001000;
...
localparam [4:0] ST_SETUP_HIDDEN_READ  = 5'd4;
localparam [4:0] ST_READ_HIDDEN        = 5'd5;
localparam [4:0] ST_SETUP_OPROJ_READ   = 5'd6;
localparam [4:0] ST_READ_OPROJ         = 5'd7;
...
wire [8:0] hidden_beats_remaining = 9'd56 - {1'b0, read_base_beat};
wire [8:0] oproj_beats_remaining  = 9'd224 - {1'b0, read_base_beat};
wire [4:0] next_hidden_burst_beats =
    (hidden_beats_remaining > 9'd16) ? 5'd16 : hidden_beats_remaining[4:0];
wire [4:0] next_oproj_burst_beats =
    (oproj_beats_remaining > 9'd16) ? 5'd16 : oproj_beats_remaining[4:0];
...
attention_residual_core u_attention_residual_core (
    .clk               (core_clk),
    .rst_n             (core_rst_n),
    .hidden_load_en    ((state == ST_READ_HIDDEN) && read_data_handshake),
    .hidden_load_index (read_base_beat + read_beat_index),
    .hidden_load_data  (axi_rdata),
    .oproj_load_en     ((state == ST_READ_OPROJ) && read_data_handshake),
    .oproj_load_index  (read_base_beat + read_beat_index),
    .oproj_load_data   (axi_rdata),
    .start             (core_start),
    .busy              (core_busy),
    .done              (core_done),
...
function [7:0] info_char;
    ...
    begin
        case (index)
            ...
            5'd23: info_char = "V";
            5'd24: info_char = "1";
            5'd25: info_char = 8'h0d;
            5'd26: info_char = 8'h0a;
    ...
    if (!core_rst_n) begin
        ...
        case (state)
            ...
            ST_SETUP_LOAD_WRITE: begin
                if (load_beat_index < HIDDEN_BEATS)
                    axi_awaddr <= ADDR_HIDDEN + (load_beat_index << 3);
                else
                    axi_awaddr <= ADDR_OPROJ +
                        ((load_beat_index - HIDDEN_BEATS) << 3);
                axi_awvalid <= 1'b1;
            ...
            ST_SETUP_HIDDEN_READ: begin
                axi_araddr              <= ADDR_HIDDEN + (read_base_beat << 3);
                axi_arlen               <= next_hidden_burst_beats - 1'b1;
                axi_arvalid             <= 1'b1;
                ar_seen                 <= 1'b0;
                read_beat_index         <= 5'd0;
                active_read_burst_beats <= next_hidden_burst_beats;
            ...
            ST_READ_HIDDEN: begin
                ...
                if (read_data_handshake) begin
                    if (read_beat_index + 1'b1 == active_read_burst_beats) begin
                        ar_seen <= 1'b0;
                        if (read_base_beat + active_read_burst_beats == HIDDEN_BEATS) begin
                            read_base_beat <= 8'd0;
                            state <= ST_SETUP_OPROJ_READ;
                        end else begin
            ...
            ST_SETUP_OPROJ_READ: begin
                axi_araddr              <= ADDR_OPROJ + (read_base_beat << 3);
                axi_arlen               <= next_oproj_burst_beats - 1'b1;
                axi_arvalid             <= 1'b1;
                ar_seen                 <= 1'b0;
                read_beat_index         <= 5'd0;
                active_read_burst_beats <= next_oproj_burst_beats;
                state                   <= ST_READ_OPROJ;
            end
            ...
            ST_READ_OPROJ: begin
                ...
                if (read_data_handshake) begin
                    if (read_beat_index + 1'b1 == active_read_burst_beats) begin
                        ar_seen <= 1'b0;
                        if (read_base_beat + active_read_burst_beats == OPROJ_BEATS)
                            state <= ST_START_CORE;
                        else begin
                            read_base_beat <= read_base_beat + active_read_burst_beats;
                            state <= ST_SETUP_OPROJ_READ;
