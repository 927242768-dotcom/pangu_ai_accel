...
// layer0 MLP 第二处残差的 UART、DDR3 和计算调度控制器。
//
// UART 协议（115200 8N1）：
...
//   down_proj_q28[896]       : 7168 B / 224 拍
module mlp_residual_ctrl #(
...
localparam integer DOWN_BEATS       = 224;
localparam integer RESULT_BEATS     = 56;
localparam integer LOAD_TOTAL_BEATS = HIDDEN_BEATS + DOWN_BEATS;

// DDR3 Controller 地址单位为 32 bit；每个 256 bit beat 地址递增 8。
localparam [CTRL_ADDR_WIDTH-1:0] ADDR_HIDDEN = 28'h0000000;
localparam [CTRL_ADDR_WIDTH-1:0] ADDR_DOWN   = 28'h0001000;
...
localparam [4:0] ST_SETUP_DOWN_READ    = 5'd6;
localparam [4:0] ST_READ_DOWN          = 5'd7;
...
wire [8:0] down_beats_remaining   = 9'd224 - {1'b0, read_base_beat};
wire [4:0] next_hidden_burst_beats =
    (hidden_beats_remaining > 9'd16) ? 5'd16 : hidden_beats_remaining[4:0];
wire [4:0] next_down_burst_beats =
    (down_beats_remaining > 9'd16) ? 5'd16 : down_beats_remaining[4:0];
...
mlp_residual_core u_mlp_residual_core (
    .clk               (core_clk),
    .rst_n             (core_rst_n),
    ...
    .down_load_en      ((state == ST_READ_DOWN) && read_data_handshake),
    .down_load_index   (read_base_beat + read_beat_index),
    .down_load_data    (axi_rdata),
    .start             (core_start),
    .busy              (core_busy),
    .done              (core_done),
...
always @(posedge core_clk or negedge core_rst_n) begin
    if (!core_rst_n) begin
        ...
        case (state)
            ...
            ST_SETUP_DOWN_READ: begin
                axi_araddr              <= ADDR_DOWN + (read_base_beat << 3);
                axi_arlen               <= next_down_burst_beats - 1'b1;
                axi_arvalid             <= 1'b1;
                ar_seen                 <= 1'b0;
                read_beat_index         <= 5'd0;
                active_read_burst_beats <= next_down_burst_beats;
                state                   <= ST_READ_DOWN;
            end
            ...
            ST_READ_DOWN: begin
                ...
                if (read_data_handshake) begin
                    if (read_beat_index + 1'b1 == active_read_burst_beats) begin
                        ar_seen <= 1'b0;
                        if (read_base_beat + active_read_burst_beats == DOWN_BEATS)
                            state <= ST_START_CORE;
                        else begin
                            read_base_beat <= read_base_beat + active_read_burst_beats;
                            state <= ST_SETUP_DOWN_READ;
