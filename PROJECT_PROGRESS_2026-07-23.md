...
这证明 DDR3 Controller + PHY、长 burst、片上缓存、INT4 解包、MAC16 分块循环、跨块 INT32 累加、自动地址调度、输出批量回写和 Python 自动验证可以协同工作。
...
1. 分别统计激活/权重 DDR3 读取周期。
2. 统计 MAC 计算周期。
3. 统计单次 GEMV 总周期。
4. 通过 UART 返回性能计数器。
5. Python 计算实测带宽、GMAC/s 和利用率。
6. 明确当前瓶颈来自 DDR3、MAC 数量还是控制开销。
