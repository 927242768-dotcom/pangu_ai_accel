...
// 盘古 Logos PGL50H K=896 元素级算子验证顶层。
// 顶层端口保持与已验证 DDR3 example design 一致，复用原有引脚和时序约束。
module elementwise_k896_top #(
...
elementwise_k896_ctrl #(
    .CTRL_ADDR_WIDTH (CTRL_ADDR_WIDTH),
    .CLKS_PER_BIT    (868)
) u_elementwise_k896_ctrl (
    .core_clk        (core_clk),
...
wire _unused_debug = &{1'b0, debug_data, debug_slice_state, debug_calib_ctrl,
                       ...
                       update_com_val_err_flag, ctrl_state, loaded, configured};
