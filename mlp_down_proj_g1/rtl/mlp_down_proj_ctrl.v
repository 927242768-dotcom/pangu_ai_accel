...
// Qwen2.5-0.5B layer0 MLP down_proj 控制器。
// 固定 M=896、K=4864、group_size=64、groups=76。
//
// UART 协议（115200 8N1）：
//   I -> "PANGU50K MLP DOWN V1\r\n"
//   S -> 'S' + 状态字节 + "\r\n"
//   L + 固定 2499328 字节载荷 -> 逐拍写入 DDR3，回复 "K\r\n"
//   G -> 执行完整 down_proj，回复 'R' + 896 个 little-endian signed int64 Q28
//
// 上传载荷：
//   activation_int8                            4864 B（152 拍）
//   packed_weight_int4[896][4864]           2179072 B（每行 76 拍）
//   combined_scale_uq4_28[896][76]           286720 B（每行补齐为 10 拍）
//   bias_q28[896]                              28672 B（每行低 64 位有效，补齐为 1 拍）
//
// AXI burst 最大 16 拍，因此激活和每一行 76 拍权重均自动分段读取。
// 计算期间只缓存完整激活与当前一行权重/scale；每 4 行结果组成一拍写回。
module mlp_down_proj_ctrl #(
...
localparam integer K = 4864;
localparam integer ACT_BEATS = 152;
localparam integer WEIGHT_BEATS_PER_ROW = 76;
localparam integer WEIGHT_TOTAL_BEATS = M * WEIGHT_BEATS_PER_ROW;
localparam integer SCALE_BEATS_PER_ROW = 10;
...
// ACT:    0x0000000 .. 0x00004bf
// WEIGHT: 0x0001000 .. 0x0085fff
// SCALE:  0x0090000 .. 0x00a17ff
// BIAS:   0x00a4000 .. 0x00a5bff
// RESULT: 0x00a8000 .. 0x00a86ff
localparam [CTRL_ADDR_WIDTH-1:0] ADDR_ACT    = 28'h0000000;
localparam [CTRL_ADDR_WIDTH-1:0] ADDR_WEIGHT = 28'h0001000;
localparam [CTRL_ADDR_WIDTH-1:0] ADDR_SCALE  = 28'h0090000;
localparam [CTRL_ADDR_WIDTH-1:0] ADDR_BIAS   = 28'h00a4000;
localparam [CTRL_ADDR_WIDTH-1:0] ADDR_RESULT = 28'h00a8000;
...
wire [8:0] act_beats_remaining = ACT_BEATS - act_read_base_beat;
wire [4:0] next_act_burst_beats =
    (act_beats_remaining > 9'd16) ? 5'd16 : act_beats_remaining[4:0];
wire [7:0] weight_beats_remaining = WEIGHT_BEATS_PER_ROW - weight_read_base_beat;
...
mlp_down_proj_core u_mlp_down_proj_core (
    .clk                   (core_clk),
    .rst_n                 (core_rst_n),
    ...
    if (!core_rst_n) begin
        ...
        case (state)
            ...
            ST_SETUP_ACT_READ: begin
                axi_araddr              <= ADDR_ACT + (act_read_base_beat * 8);
            ...
            ST_SETUP_RESULT_READ: begin
                axi_araddr      <= ADDR_RESULT + (result_read_beat_index * 8);
                axi_arlen       <= 4'd0;
                axi_arvalid     <= 1'b1;
                ar_seen         <= 1'b0;
