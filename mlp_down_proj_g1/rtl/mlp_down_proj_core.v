...
// Qwen2.5-0.5B layer0 MLP down_proj 单行分组 Q28 GEMV 核心。
//
// 每个输出行固定 K=4864、group_size=64、groups=76：
...
//
// 输入激活来自已经验证的 SiLU(gate)×up Q28，由主机按逐向量对称规则量化
// 为 INT8。真实 down_proj 无 bias，但保留通用 bias_q28 端口并固定上传全零。
module mlp_down_proj_core #(
    parameter integer K             = 4864,
...
// scale 是 unsigned uint32。零扩展为正的 signed 33 位后参与有符号乘法，
// 防止 bit31=1 被解释为负数。
...
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        case (state)
            ...
            ST_WAIT_MAC: begin
                if (dot_valid) begin
                    if (block_index[1:0] == 2'd3) begin
                        ...
                        scale_reg               <= scale_mem[block_index[8:2]];
                        group_accumulator       <= 32'sd0;
                        state                   <= ST_MULTIPLY;
                    end else begin
            ...
            ST_MULTIPLY: begin
                // 软件已证明 76 组最坏绝对累加小于 signed int64 上限；
                // 单组乘积也严格落入 signed int64，因此此处无隐含溢出。
