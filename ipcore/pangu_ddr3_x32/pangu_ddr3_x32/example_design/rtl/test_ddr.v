...
// The MES50HP board provides one verified 50 MHz global clock on P20.
// Reuse it for the low-speed UART/debug clock as in the official board demo.
wire                             free_clk;
...
begin
   if (!resetn)
      heart_beat_led <= 1'b0;
   else if (test_main_state == 4'd5)
      heart_beat_led <= 1'b1; // full-address write/read PASS
   else if (test_main_state == 4'd6)
      heart_beat_led <= 1'b0; // FAIL is also latched on err_flag_led
   else if (cnt >= TH_1S)
      heart_beat_led <= ~heart_beat_led; // test is still running
end
