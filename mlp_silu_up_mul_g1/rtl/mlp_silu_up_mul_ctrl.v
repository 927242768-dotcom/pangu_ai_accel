...
// layer0 MLP SiLU(gate) × up UART、DDR3 与计算调度控制器。
//
// UART 协议（115200 8N1）：
//   I -> "PANGU50K MLP SILUUP V1\r\n"
//   S -> 'S' + flags + "\r\n"
//   L + 48640 B 固定载荷 -> "K\r\n"
//   G -> 计算并回复 'R' + 4864 个 little-endian signed int64 Q28
//
// 上传载荷：
//   SiLU(gate) Q6.10 [4864] :  9728 B /  304 拍
//   up_proj Q28 [4864]      : 38912 B / 1216 拍
module mlp_silu_up_mul_ctrl #(
...
localparam integer SILU_BEATS       = 304;
localparam integer UP_BEATS         = 1216;
localparam integer RESULT_BEATS     = 1216;
localparam integer LOAD_TOTAL_BEATS = SILU_BEATS + UP_BEATS;

// DDR3 Controller 地址单位为 32 bit；每个 256 bit beat 地址递增 8。
localparam [CTRL_ADDR_WIDTH-1:0] ADDR_SILU   = 28'h0000000;
localparam [CTRL_ADDR_WIDTH-1:0] ADDR_UP     = 28'h0001000;
localparam [CTRL_ADDR_WIDTH-1:0] ADDR_RESULT = 28'h0004000;

localparam [4:0] ST_IDLE               = 5'd0;
localparam [4:0] ST_RECV_LOAD          = 5'd1;
localparam [4:0] ST_SETUP_LOAD_WRITE   = 5'd2;
localparam [4:0] ST_WRITE_LOAD         = 5'd3;
localparam [4:0] ST_SETUP_SILU_READ    = 5'd4;
localparam [4:0] ST_READ_SILU          = 5'd5;
localparam [4:0] ST_SETUP_UP_READ      = 5'd6;
localparam [4:0] ST_READ_UP            = 5'd7;
localparam [4:0] ST_START_CORE         = 5'd8;
localparam [4:0] ST_WAIT_CORE_RESULT   = 5'd9;
localparam [4:0] ST_SETUP_RESULT_WRITE = 5'd10;
localparam [4:0] ST_WRITE_RESULT       = 5'd11;
localparam [4:0] ST_SETUP_RESULT_READ  = 5'd12;
localparam [4:0] ST_READ_RESULT        = 5'd13;
localparam [4:0] ST_SEND_RESULT_PREFIX = 5'd14;
localparam [4:0] ST_SEND_RESULT_BYTES  = 5'd15;
localparam [4:0] ST_SEND_INFO          = 5'd16;
localparam [4:0] ST_SEND_STATUS        = 5'd17;
localparam [4:0] ST_SEND_ACK           = 5'd18;
localparam [4:0] ST_SEND_ERROR         = 5'd19;
...
reg [10:0] load_beat_index;

reg [10:0] read_base_beat;
reg [4:0] active_read_burst_beats;
reg [4:0] read_beat_index;

reg [10:0] result_write_beat_index;
reg [255:0] result_write_cache;
reg [10:0] result_read_beat_index;
...
wire [11:0] silu_beats_remaining = 12'd304 - {1'b0, read_base_beat};
wire [11:0] up_beats_remaining = 12'd1216 - {1'b0, read_base_beat};
wire [4:0] next_silu_burst_beats =
    (silu_beats_remaining > 12'd16) ? 5'd16 : silu_beats_remaining[4:0];
wire [4:0] next_up_burst_beats =
    (up_beats_remaining > 12'd16) ? 5'd16 : up_beats_remaining[4:0];
...
mlp_silu_up_mul_core u_mlp_silu_up_mul_core (
    .clk              (core_clk),
    .rst_n            (core_rst_n),
    .silu_load_en     ((state == ST_READ_SILU) && read_data_handshake),
    .silu_load_index  (read_base_beat[8:0] + read_beat_index),
    .silu_load_data   (axi_rdata),
    .up_load_en       ((state == ST_READ_UP) && read_data_handshake),
    .up_load_index    (read_base_beat + read_beat_index),
    .up_load_data     (axi_rdata),
    .start            (core_start),
    .busy             (core_busy),
    .done             (core_done),
...
always @(posedge core_clk or negedge core_rst_n) begin
    if (!core_rst_n) begin
        ...
        rx_byte_index            <= 6'd0;
        upload_beat              <= 256'd0;
        load_beat_index          <= 11'd0;
        read_base_beat           <= 11'd0;
        active_read_burst_beats  <= 5'd0;
        read_beat_index          <= 5'd0;
        result_write_beat_index  <= 11'd0;
        result_write_cache       <= 256'd0;
        result_read_beat_index   <= 11'd0;
        ...
        case (state)
            ...
            ST_SETUP_LOAD_WRITE: begin
                if (load_beat_index < SILU_BEATS)
                    axi_awaddr <= ADDR_SILU + (load_beat_index << 3);
                else
                    axi_awaddr <= ADDR_UP + ((load_beat_index - SILU_BEATS) << 3);
                axi_awvalid <= 1'b1;
            ...
            ST_SETUP_SILU_READ: begin
                axi_araddr              <= ADDR_SILU + (read_base_beat << 3);
                axi_arlen               <= next_silu_burst_beats - 1'b1;
                axi_arvalid             <= 1'b1;
                ar_seen                 <= 1'b0;
                read_beat_index         <= 5'd0;
                active_read_burst_beats <= next_silu_burst_beats;
                state                   <= ST_READ_SILU;
            end
            ...
            ST_READ_SILU: begin
                ...
                if (read_data_handshake) begin
                    if (read_beat_index + 1'b1 == active_read_burst_beats) begin
                        ar_seen <= 1'b0;
                        if (read_base_beat + active_read_burst_beats == SILU_BEATS) begin
                            read_base_beat <= 11'd0;
            ...
            ST_SETUP_UP_READ: begin
                axi_araddr              <= ADDR_UP + (read_base_beat << 3);
                axi_arlen               <= next_up_burst_beats - 1'b1;
                axi_arvalid             <= 1'b1;
                ar_seen                 <= 1'b0;
                read_beat_index         <= 5'd0;
                active_read_burst_beats <= next_up_burst_beats;
                state                   <= ST_READ_UP;
            end
            ...
            ST_READ_UP: begin
                ...
                if (read_data_handshake) begin
                    if (read_beat_index + 1'b1 == active_read_burst_beats) begin
                        ar_seen <= 1'b0;
                        if (read_base_beat + active_read_burst_beats == UP_BEATS) begin
                            read_base_beat <= 11'd0;
                            state <= ST_START_CORE;
                        end else begin
                            read_base_beat <= read_base_beat + active_read_burst_beats;
