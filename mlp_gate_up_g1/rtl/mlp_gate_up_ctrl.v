...
// Qwen2.5-0.5B layer0 MLP gate_proj/up_proj 通用完整投影控制器。
//
// 固定 M=4864、K=896、group_size=64。gate_proj 与 up_proj 共享完全相同的
// 数据通路和载荷布局，由上位机分别上传真实权重/scale 后独立运行。
// 两个真实投影均无 bias，载荷中的 padded bias_q28 必须全零。
//
// UART 协议（115200 8N1）：
//   I -> "PANGU50K MLP GATEUP V1\r\n"
//   S -> 'S' + 状态字节 + "\r\n"
//   L + 固定 2646912 字节载荷 -> 逐拍写入 DDR3，回复 "K\r\n"
//   G -> 执行当前已加载投影，回复 'R' + 4864 个 little-endian signed int64 Q28
//
// 上传载荷：
//   activation_int8                           896 B（28 拍）
//   packed_weight_int4[4864][896]         2179072 B（每行 14 拍）
//   combined_scale_uq4_28[4864][14]        311296 B（每行补齐为 2 拍）
//   bias_q28[4864]                          155648 B（每行 1 拍，全零）
//
// 计算期间只缓存完整激活和当前一行权重/scale。每 4 行结果组成一个 256 bit
// 数据拍立即写回 DDR3；返回阶段再从 DDR3 逐拍读取并经 UART 流式发送。
module mlp_gate_up_ctrl #(
...
// 区域边界（控制器地址）：weight 结束 0x086000，scale 结束 0x0a3000，
// bias 结束 0x0ad800，result 结束 0x0b2600，彼此均不重叠。
...
localparam [CTRL_ADDR_WIDTH-1:0] ADDR_RESULT = 28'h00b0000;
...
// 复用已经真实上板验证的 K=896、group=64 Q28 Linear 核。
gemv_qproj_full_core u_mlp_projection_core (
    .clk                   (core_clk),
    .rst_n                 (core_rst_n),
