...
// 盘古 Logos PGL50H layer0 Attention O_proj 独立验证顶层。
//
// 直接复用已经真实上板验证的完整 M=896、K=896 分组 Q28 Linear
// controller/core，同时保持 DDR3 IP 实例名 I_ipsxb_ddr_top 位于顶层，确保
// 原开发板 DDR3、时钟、UART 和 LED 约束可以不经修改直接复用。
module attention_oproj_top #(
...
gemv_qproj_full_ctrl #(
    .CTRL_ADDR_WIDTH (CTRL_ADDR_WIDTH),
    .CLKS_PER_BIT    (868)
) u_gemv_qproj_full_ctrl (
    .core_clk        (core_clk),
...
wire _unused_debug = &{1'b0, debug_data, debug_slice_state, debug_calib_ctrl,
                       ...
                       update_com_val_err_flag, ctrl_state, loaded};
