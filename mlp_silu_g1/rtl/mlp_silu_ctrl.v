...
// layer0 MLP SiLU(gate) UART、DDR3 与计算调度控制器。
//
// UART 协议（115200 8N1）：
//   I -> "PANGU50K MLP SILU V1\r\n"
//   S -> 'S' + flags + "\r\n"
//   L + 39072 B 固定载荷 -> "K\r\n"
//   G -> 计算并回复 'R' + 4864 个 little-endian signed int16 Q6.10
//
// 上传载荷：
//   gate_q28[4864]          : 38912 B / 1216 拍
//   PWL64 endpoints padded  :   160 B /    5 拍
module mlp_silu_ctrl #(
...
localparam integer GATE_BEATS       = 1216;
localparam integer PWL_BEATS        = 5;
localparam integer RESULT_BEATS     = 304;
localparam integer LOAD_TOTAL_BEATS = GATE_BEATS + PWL_BEATS;

// DDR3 Controller 地址单位为 32 bit；每个 256 bit beat 地址递增 8。
localparam [CTRL_ADDR_WIDTH-1:0] ADDR_GATE   = 28'h0000000;
localparam [CTRL_ADDR_WIDTH-1:0] ADDR_PWL    = 28'h0003000;
...
localparam [4:0] ST_SETUP_GATE_READ    = 5'd4;
localparam [4:0] ST_READ_GATE          = 5'd5;
localparam [4:0] ST_SETUP_PWL_READ     = 5'd6;
localparam [4:0] ST_READ_PWL           = 5'd7;
...
reg [8:0] result_write_beat_index;
...
wire [11:0] gate_beats_remaining = 12'd1216 - {1'b0, read_base_beat};
wire [4:0] next_gate_burst_beats =
    (gate_beats_remaining > 12'd16) ? 5'd16 : gate_beats_remaining[4:0];
...
mlp_silu_core u_mlp_silu_core (
    .clk              (core_clk),
    .rst_n            (core_rst_n),
    .gate_load_en     ((state == ST_READ_GATE) && read_data_handshake),
    .gate_load_index  (read_base_beat + read_beat_index),
    .gate_load_data   (axi_rdata),
    .pwl_load_en      ((state == ST_READ_PWL) && read_data_handshake),
    .pwl_load_index   (read_beat_index[2:0]),
    ...
    if (!core_rst_n) begin
        ...
        case (state)
            ...
            ST_SETUP_LOAD_WRITE: begin
                if (load_beat_index < GATE_BEATS)
                    axi_awaddr <= ADDR_GATE + (load_beat_index << 3);
                else
                    axi_awaddr <= ADDR_PWL + ((load_beat_index - GATE_BEATS) << 3);
                axi_awvalid <= 1'b1;
            ...
            ST_SETUP_GATE_READ: begin
                axi_araddr              <= ADDR_GATE + (read_base_beat << 3);
                axi_arlen               <= next_gate_burst_beats - 1'b1;
                axi_arvalid             <= 1'b1;
                ar_seen                 <= 1'b0;
                read_beat_index         <= 5'd0;
                active_read_burst_beats <= next_gate_burst_beats;
                state                   <= ST_READ_GATE;
            end
            ...
            ST_READ_GATE: begin
                ...
                if (read_data_handshake) begin
                    if (read_beat_index + 1'b1 == active_read_burst_beats) begin
                        ar_seen <= 1'b0;
                        if (read_base_beat + active_read_burst_beats == GATE_BEATS) begin
                            read_base_beat <= 11'd0;
            ...
            ST_SETUP_PWL_READ: begin
                ...
                ar_seen                 <= 1'b0;
                read_beat_index         <= 5'd0;
                active_read_burst_beats <= PWL_BEATS;
                state                   <= ST_READ_PWL;
            end
