...
// 盘古 Logos PGL50H F4 Attention Score 独立验证顶层。
// 顶层端口保持与已验证 DDR3 example design 一致，复用原有引脚和时序约束。
module attention_score_top #(
...
wire [15:0] current_query_position;
wire protocol_error;
wire configured;
wire q_loaded;
wire result_valid;
...
attention_score_ctrl #(
    .CTRL_ADDR_WIDTH (CTRL_ADDR_WIDTH),
    .CLKS_PER_BIT    (868),
    .NUM_LAYERS      (28),
    .MAX_CONTEXT     (16384),
    .MAX_TOKENS      (16)
) u_attention_score_ctrl (
    .core_clk             (core_clk),
    ...
    .debug_query_position (current_query_position),
...
wire _unused_debug = &{1'b0, debug_data, debug_slice_state, debug_calib_ctrl,
                       ...
                       current_query_position, configured, q_loaded};
