...
```text
进入 F4 Attention Score：基于 F1 的 Q、F2 的 RoPE 和 F3 的历史 K Cache，
先建立 Q·K 点积、1/sqrt(head_dim)=1/8 定点缩放和 causal mask 软件金标准；
再新建独立硬件工程，实现 14 个 Q heads 对对应 2 个 KV heads 的 GQA 映射、
历史 token 循环读取和多头调度，并完成固定/随机序列、PDS、多角时序和真实上板验证。
不得进入 Softmax，且不得覆盖 F3 及更早阶段的验证工程和位流。
```
