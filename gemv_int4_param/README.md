# PGL50H 运行时参数化 packed INT4 GEMV 验证工程
...
- 参数化 GEMV：`pnr/generate_bitstream/gemv_param_top.sbit`
...
D1.2 参数化 GEMV 已完成。下一步进入 D1.3 GEMV 性能基础设施：分别统计 DDR3 读取、MAC 计算和总执行周期，通过 UART 读取性能计数器，并计算实测带宽、GMAC/s 和利用率。
