...
// PGL50H G2 Q6.10/Q28 运行时量化自动逐位闭环板级顶层。
// DDR3 IP 实例名保持 I_ipsxb_ddr_top，以复用已真实上板验证的 FDC。
module runtime_quantizer_validation_top #(
...
ipsxb_rst_sync_v1_1 u_core_clk_rst_sync (
    .clk(core_clk), .rst_n(resetn), .sig_async(1'b1), .sig_synced(core_clk_rst_n)
);
...
pangu_ddr3_x32 #(
    ...
    .ref_clk(ref_clk), .resetn(resetn), .ddr_init_done(ddr_init_done),
    .ddrphy_clkin(core_clk), .pll_lock(pll_lock),
    ...
    .axi_wready(axi_wready), .axi_wusero_id(), .axi_wusero_last(),
    ...
    .axi_rdata(axi_rdata), .axi_rid(), .axi_rlast(), .axi_rvalid(axi_rvalid),

    .apb_clk(1'b0), .apb_rst_n(1'b0), .apb_sel(1'b0), .apb_enable(1'b0),
    .apb_addr(8'd0), .apb_write(1'b0), .apb_ready(),
    .apb_wdata(16'd0), .apb_rdata(), .apb_int(),

    .debug_data(debug_data), .debug_slice_state(debug_slice_state),
    .debug_calib_ctrl(debug_calib_ctrl), .ck_dly_set_bin(ck_dly_set_bin),
    .ck_dly_en(1'b1), .init_ck_dly_step(8'h30),
    .wl_step_ov_warning(wl_step_ov_warning), .dll_step(dll_step),
    .dll_lock(dll_lock),
    .init_read_clk_ctrl({(2*MEM_DQS_WIDTH){1'b0}}),
    .init_slip_step({(4*MEM_DQS_WIDTH){1'b0}}),
    .force_read_clk_ctrl(1'b0), .ddrphy_gate_update_en(1'b1),
    .update_com_val_err_flag(update_com_val_err_flag), .rd_fake_stop(1'b0),
...
runtime_quantizer_validation_ctrl #(
    .CTRL_ADDR_WIDTH(CTRL_ADDR_WIDTH), .CLKS_PER_BIT(868)
) u_runtime_quantizer_validation_ctrl (
    .core_clk(core_clk), .core_rst_n(core_clk_rst_n),
    .ddr_init_done(ddr_init_done), .uart_rx_i(uart_rxd), .uart_tx_o(uart_txd),
    .axi_awaddr(axi_awaddr), .axi_awuser_ap(axi_awuser_ap),
    .axi_awuser_id(axi_awuser_id), .axi_awlen(axi_awlen),
    .axi_awready(axi_awready), .axi_awvalid(axi_awvalid),
    ...
    .axi_aruser_id(axi_aruser_id), .axi_arlen(axi_arlen),
    .axi_arready(axi_arready), .axi_arvalid(axi_arvalid),
    .axi_rdata(axi_rdata), .axi_rvalid(axi_rvalid),
    .debug_state(ctrl_state), .protocol_error(protocol_error),
    .configured(configured), .loaded(loaded), .result_valid(result_valid)
);

wire _unused_debug = &{1'b0, debug_data, debug_slice_state, debug_calib_ctrl,
    ck_dly_set_bin, wl_step_ov_warning, dll_step, dll_lock,
    update_com_val_err_flag, ctrl_state, configured, loaded};
