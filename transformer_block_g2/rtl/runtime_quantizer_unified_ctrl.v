...
// G2 完整 Block 专用单矩阵统一运行时量化阶段：
//   1. Q6.10/Q28 DDR3 source -> 单个 Q28 activation quantizer -> INT8 scratch；
//   2. raw FP16 weight scale -> padded UQ4.28 combined scale。
module runtime_quantizer_unified_ctrl #(
...
runtime_activation_quantizer_stream_ctrl #(
    .CTRL_ADDR_WIDTH(CTRL_ADDR_WIDTH)
) u_runtime_activation_quantizer_stream_ctrl (
    .clk                     (clk),
    .rst_n                   (rst_n),
    .ddr_init_done           (ddr_init_done),
    .start                   (act_start),
    .source_q28              (cfg_source_q28),
...
wire _unused_status = &{
    1'b0,
    act_busy, act_debug_state,
    scale_busy, scale_debug_state,
    scale_current_row, scale_current_group
};
