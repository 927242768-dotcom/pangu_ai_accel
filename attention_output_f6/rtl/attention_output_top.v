...
// 盘古 Logos PGL50H F6 Attention 输出独立验证顶层。
// 顶层端口保持与已验证 DDR3 example design 一致，复用原有引脚和时序约束。
module attention_output_top #(
...
wire [6:0] ctrl_state;
wire [4:0] current_layer;
wire [15:0] current_window_start;
wire protocol_error;
wire configured;
wire probabilities_loaded;
wire result_valid;
...
assign heart_beat_led = result_valid ? 1'b1 :
                        (ddr_init_done ? heartbeat : 1'b0);
...
attention_output_ctrl #(
    .CTRL_ADDR_WIDTH (CTRL_ADDR_WIDTH),
    .CLKS_PER_BIT    (868),
    .NUM_LAYERS      (28),
    .MAX_CONTEXT     (16384),
    .MAX_TOKENS      (16)
) u_attention_output_ctrl (
    .core_clk             (core_clk),
    ...
    .debug_layer          (current_layer),
    .debug_window_start   (current_window_start),
    .protocol_error       (protocol_error),
    .configured           (configured),
    .probabilities_loaded (probabilities_loaded),
...
wire _unused_debug = &{1'b0, debug_data, debug_slice_state, debug_calib_ctrl,
                       ...
                       update_com_val_err_flag, ctrl_state, current_layer,
                       current_window_start, configured, probabilities_loaded};
