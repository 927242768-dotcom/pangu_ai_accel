...
// layer0 Q/K/V 完整真实 Linear 统一控制器。
//
// 固定 K=896、group_size=64；运行时投影类型决定输出行数：
//   Q: M=896, 14 heads x 64
//   K: M=128,  2 heads x 64
//   V: M=128,  2 heads x 64
//
// UART 协议（115200 8N1）：
//   I -> "PANGU50K QKV LINEAR V1\r\n"
//   S -> 'S' + 状态字节 + "\r\n"
//   Q/K/V -> 选择投影，清除 loaded/result_valid，回复 "K\r\n"
//   L + 当前投影固定长度载荷 -> 写入 DDR3，回复 "K\r\n"
//   G -> 执行当前投影，回复 'R' + M 个 little-endian signed int64 Q28
//
// Q 载荷 488320 B；K/V 载荷 70528 B。三者均为：
// activation + packed weight + padded UQ4.28 scale + padded Q28 bias。
module qkv_linear_ctrl #(
    ...
    output wire [1:0]                   debug_projection,
...
localparam [1:0] PROJ_Q = 2'd0;
localparam [1:0] PROJ_K = 2'd1;
localparam [1:0] PROJ_V = 2'd2;

localparam [9:0] Q_ROWS  = 10'd896;
localparam [9:0] KV_ROWS = 10'd128;

localparam [13:0] Q_WEIGHT_TOTAL_BEATS  = 14'd12544;
localparam [13:0] KV_WEIGHT_TOTAL_BEATS = 14'd1792;
localparam [13:0] Q_SCALE_TOTAL_BEATS   = 14'd1792;
localparam [13:0] KV_SCALE_TOTAL_BEATS  = 14'd256;
localparam [13:0] Q_BIAS_TOTAL_BEATS    = 14'd896;
localparam [13:0] KV_BIAS_TOTAL_BEATS   = 14'd128;
localparam [13:0] Q_LOAD_TOTAL_BEATS    = 14'd15260;
localparam [13:0] KV_LOAD_TOTAL_BEATS   = 14'd2204;
localparam [7:0]  Q_RESULT_BEATS        = 8'd224;
localparam [7:0]  KV_RESULT_BEATS       = 8'd32;

// DDR3 控制器地址单位为 32 bit；一个 256 bit 数据拍占 8 个地址单位。
// 复用已验证 q_proj 地址布局，K/V 仅使用各区域前部。
...
wire projection_is_q = (projection_select == PROJ_Q);
wire [9:0] active_rows = projection_is_q ? Q_ROWS : KV_ROWS;
wire [13:0] active_weight_total_beats =
    projection_is_q ? Q_WEIGHT_TOTAL_BEATS : KV_WEIGHT_TOTAL_BEATS;
wire [13:0] active_scale_total_beats =
    projection_is_q ? Q_SCALE_TOTAL_BEATS : KV_SCALE_TOTAL_BEATS;
wire [13:0] active_bias_total_beats =
    projection_is_q ? Q_BIAS_TOTAL_BEATS : KV_BIAS_TOTAL_BEATS;
wire [13:0] active_load_total_beats =
    projection_is_q ? Q_LOAD_TOTAL_BEATS : KV_LOAD_TOTAL_BEATS;
wire [7:0] active_result_beats =
    projection_is_q ? Q_RESULT_BEATS : KV_RESULT_BEATS;

wire [13:0] weight_region_end = 14'd28 + active_weight_total_beats;
wire [13:0] scale_region_end = weight_region_end + active_scale_total_beats;
wire [13:0] bias_region_end = scale_region_end + active_bias_total_beats;
...
assign debug_projection = projection_select;
...
qkv_linear_core u_qkv_linear_core (
    .clk                   (core_clk),
    .rst_n                 (core_rst_n),
    ...
    if (!core_rst_n) begin
        ...
        case (state)
                ...
                if (rx_valid) begin
                    case (rx_data)
                        ...
                        8'h53, 8'h73: begin // S / s
                            status_snapshot <= {
                                2'd0, projection_select,
                                core_busy, result_valid, loaded, ddr_init_done
                            };
                        ...
                        8'h4c, 8'h6c: begin // L / l
                            if (!ddr_init_done) begin
                                ...
                                error_code     <= 8'h02;
                                state          <= ST_SEND_ERROR;
                            end else if (projection_select == 2'd3) begin
                                protocol_error <= 1'b1;
                                error_code     <= 8'h05;
                                state          <= ST_SEND_ERROR;
                            end else begin
            ...
            ST_SETUP_LOAD_WRITE: begin
                ...
                end else if (load_beat_index < weight_region_end) begin
                    axi_awaddr <= ADDR_WEIGHT +
                        ((load_beat_index - ACT_BEATS) << 3);
                end else if (load_beat_index < scale_region_end) begin
                    axi_awaddr <= ADDR_SCALE +
                        ((load_beat_index - weight_region_end) << 3);
                end else begin
                    axi_awaddr <= ADDR_BIAS +
                        ((load_beat_index - scale_region_end) << 3);
                end
                axi_awvalid <= 1'b1;
            ...
            ST_WRITE_LOAD: begin
                ...
                if ((aw_seen || aw_handshake) && (w_seen || write_data_handshake)) begin
                    ...
                    aw_seen     <= 1'b0;
                    w_seen      <= 1'b0;
                    if (load_beat_index + 1'b1 == active_load_total_beats) begin
                        loaded <= 1'b1;
                        state  <= ST_SEND_ACK;
                    end else begin
            ...
            ST_WRITE_RESULT: begin
                ...
                if ((aw_seen || aw_handshake) && (w_seen || write_data_handshake)) begin
                    ...
                    w_seen            <= 1'b0;
                    result_beat_cache <= 256'd0;
                    if (row_index + 1'b1 == active_rows) begin
                        result_valid           <= 1'b1;
                        result_read_beat_index  <= 8'd0;
                        result_tx_byte_index    <= 6'd0;
                        result_prefix_sent      <= 1'b0;
            ...
            ST_SEND_RESULT_BYTES: begin
                if (!tx_busy && !tx_start) begin
                    ...
                    if (result_tx_byte_index == 6'd31) begin
                        if (result_read_beat_index + 1'b1 == active_result_beats) begin
                            state <= ST_IDLE;
                        end else begin
