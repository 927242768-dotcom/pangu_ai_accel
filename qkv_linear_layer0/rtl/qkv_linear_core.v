...
// layer0 Q/K/V 共用的单行分组 Q28 GEMV 核心。
//
// 每个输出行固定 K=896、group_size=64：
...
//
// Q/K/V 仅输出行数不同，单行数学路径完全相同；行数由外部控制器调度。
module qkv_linear_core #(
...
// 仍按正数参与有符号乘法。当前真实模型范围保证结果可装入 signed int64。
