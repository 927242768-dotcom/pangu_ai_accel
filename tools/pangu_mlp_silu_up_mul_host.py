#!/usr/bin/env python3
"""盘古 PGL50H layer0 MLP ``SiLU(gate) × up`` 验证工具。"""
...
from model_tools.mlp_silu_up_mul_reference import (  # noqa: E402
    DEFAULT_MANIFEST,
    DEFAULT_STRESS_SEED,
    M,
    RESULT_BYTES,
...
BAUD_RATE = 115200
EXPECTED_FIRMWARE = "PANGU50K MLP SILUUP V1"

ERROR_MESSAGES = {
    0x01: "未知命令",
    0x02: "DDR3 尚未初始化完成",
    0x04: "尚未加载 SiLU×up 数据",
    0xFF: "FPGA 状态机异常",
}
...
def load_case(port: "serial.Serial", case: SiLUUpMulCase) -> None:
...
def run_loaded(port: "serial.Serial") -> np.ndarray:
    ...
    raise_if_error_frame(reply)
    if reply[0:1] != b"R":
        raise RuntimeError(f"SiLU×up 结果帧头错误：{reply[:16]!r}")
    result = np.frombuffer(reply[1:], dtype="<i8").copy()
    if result.shape != (M,):
        raise RuntimeError(f"SiLU×up 结果形状错误：{result.shape}")
    return result


def run_and_compare(port: "serial.Serial", case: SiLUUpMulCase) -> np.ndarray:
    load_case(port, case)
    fpga = run_loaded(port)
    expected = case.output_q28
...
def command_fixed(port: "serial.Serial", manifest_path: Path) -> None:
    ...
    for index, case in enumerate(cases):
        ...
        expected_hash = manifest["cases"][index]["sha256"]["output_q28"]
        actual_hash = sha256_array(fpga, "<i8")
    ...
    print(
        f"四组连贯真实 SiLU(gate) × up 固定输入全部通过，"
        f"耗时 {time.monotonic() - started:.2f} 秒"
    )


def command_stress(
    port: "serial.Serial", rounds: int, seed: int, start_index: int = 0
) -> None:
    if rounds <= 0:
    ...
    for skipped_index in range(start_index):
        make_random_inputs(rng, skipped_index)
    started = time.monotonic()
    for local_index in range(rounds):
        index = start_index + local_index
        silu_q10, up_q28 = make_random_inputs(rng, index)
        case = case_from_inputs(
            silu_q10,
            up_q28,
            label=(
                f"MLP SiLU×up FPGA stress global={index + 1} "
                f"batch={local_index + 1}/{rounds} mode={index % 8}"
            ),
        )
        run_and_compare(port, case)
        if local_index == 0 or local_index + 1 == rounds or (local_index + 1) % 10 == 0:
            print(
                f"MLP SiLU×up 真实 FPGA 已通过全局第 {index + 1} 组，"
                f"本批 {local_index + 1}/{rounds}，mode={index % 8}，seed={seed}"
            )
    print(
        f"MLP SiLU×up 真实 FPGA 随机/边界回归批次 PASS：{rounds}/{rounds}，"
        f"全局范围={start_index + 1}..{start_index + rounds}，seed={seed}，"
        f"耗时 {time.monotonic() - started:.2f} 秒"
    )
...
def command_selftest(manifest_path: Path, rounds: int, seed: int) -> None:
    ...
    cases = build_fixed_real_cases()
    manifest = validate_manifest(cases, manifest_path)
    print("MLP SiLU(gate) × up 四组真实固定清单：PASS")
    ...
    for index, case in enumerate(cases):
        ...
        print(
            ...
            f"silu={sha256_array(case.silu_q10, '<i2')} "
            f"up={sha256_array(case.up_q28, '<i8')} "
            f"output={sha256_array(case.output_q28, '<i8')}"
        )
    ...
    for index in range(rounds):
        ...
        case = case_from_inputs(
            silu_q10,
            up_q28,
            label=f"MLP SiLU×up selftest {index + 1}/{rounds}",
        )
        if index < 8:
            verify_upload_payload(case)
        if (not np.any(silu_q10) or not np.any(up_q28)) and np.any(case.output_q28):
            raise RuntimeError("零乘数没有严格输出全零")
        if index == 0 or index + 1 == rounds or (index + 1) % 100 == 0:
            print(f"MLP SiLU×up 软件随机/边界已通过 {index + 1}/{rounds}")
    print(
        f"MLP SiLU×up 软件参考与载荷压力 PASS：{rounds}/{rounds}，"
        f"seed={seed}，耗时 {time.monotonic() - started:.2f} 秒"
    )


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="PGL50H layer0 MLP SiLU(gate) × up 上位机")
    ...
    stress.add_argument(
        ...
        type=int,
        default=0,
        help="从固定随机序列的第几个样例开始，便于分批完成同一 seed 回归",
    )
