...
// 固定 M=4、K=896、group_size=64 的真实模型分组定点 GEMV 控制器。
//
// UART 协议（115200 8N1）：
//   I -> "PANGU50K GEMV Q28 V1\r\n"
//   S -> 'S' + 状态字节 + "\r\n"
//   L + 固定 2976 字节载荷 -> 写入 DDR3，回复 "K\r\n"
//   G -> 执行，回复 'R' + 4 个 little-endian signed int64 Q28
//
// 固定上传载荷：
//   activation_int8                       896 B（28 拍）
//   packed_weight_int4[4][896]           1792 B（每行 14 拍）
//   combined_scale_uq4_28[4][14]          256 B（每行补齐为 2 拍）
//   bias_q28[4]                            32 B（1 拍）
module gemv_group_q28_ctrl #(
...
localparam integer LOAD_TOTAL_BEATS =
    ACT_BEATS + WEIGHT_TOTAL_BEATS + SCALE_TOTAL_BEATS + 1;
...
localparam [CTRL_ADDR_WIDTH-1:0] ADDR_SCALE  = {{(CTRL_ADDR_WIDTH-11){1'b0}}, 11'h400};
localparam [CTRL_ADDR_WIDTH-1:0] ADDR_BIAS   = {{(CTRL_ADDR_WIDTH-11){1'b0}}, 11'h500};
localparam [CTRL_ADDR_WIDTH-1:0] ADDR_RESULT = {{(CTRL_ADDR_WIDTH-11){1'b0}}, 11'h600};
...
localparam [4:0] ST_SETUP_BIAS_READ    = 5'd6;
localparam [4:0] ST_READ_BIAS          = 5'd7;
localparam [4:0] ST_SETUP_WEIGHT_READ  = 5'd8;
localparam [4:0] ST_READ_WEIGHT        = 5'd9;
localparam [4:0] ST_SETUP_SCALE_READ   = 5'd10;
localparam [4:0] ST_READ_SCALE         = 5'd11;
...
localparam [4:0] ST_SEND_RESULT        = 5'd19;
localparam [4:0] ST_SEND_ERROR         = 5'd20;
...
wire signed [63:0] selected_bias_q28 = bias_cache[row_index*64 +: 64];
...
gemv_group_q28_core u_gemv_group_q28_core (
    .clk                   (core_clk),
    .rst_n                 (core_rst_n),
...
always @(posedge core_clk or negedge core_rst_n) begin
    if (!core_rst_n) begin
        ...
        case (state)
            ...
            ST_WAIT_CORE: begin
                if (core_done) begin
                    result_cache[row_index*64 +: 64] <= core_y_q28;
